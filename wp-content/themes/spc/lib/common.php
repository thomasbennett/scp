...
function get_post_images($post_id=null) {
    ...
    $images = get_children('post_type=attachment&post_mime_type=image&post_parent=' . $post_id . '&orderby=menu_order&order=ASC&exclude=' . get_post_thumbnail_id($post_id));
    foreach ($images as $index => $i) {
        ...
        $image_obj = wp_get_attachment_image_src($i->ID, 'single_post_thumb');
        $images[$index]->post_thumb = $image_obj[0];
	}
    return $images;
}
